import modal
import json
import os
import uuid
from datetime import datetime

app = modal.App("airbnb-calendar-book")

image = modal.Image.debian_slim(python_version="3.11").pip_install(
    "fastapi",
    "google-auth",
    "google-api-python-client",
    "google-auth-httplib2",
    "PyJWT",
    "requests",
)

# Service account credentials for Google Calendar access
# The calendar (techtomlet@gmail.com) must be shared with this service account
SERVICE_ACCOUNT_JSON = json.dumps({
    "type": "service_account",
    "project_id": "automation-reminder-481818",
    "private_key_id": "fa6c5ede3612bc5d7b38b9700a89146349be37cc",
    "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
    "client_email": "mark-496@automation-reminder-481818.iam.gserviceaccount.com",
    "client_id": "114650536455139699162",
    "auth_uri": "https://accounts.google.com/o/oauth2/auth",
    "token_uri": "https://oauth2.googleapis.com/token",
    "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
    "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/mark-496%40automation-reminder-481818.iam.gserviceaccount.com",
    "universe_domain": "googleapis.com"
})

CALENDAR_ID = "techtomlet@gmail.com"


def get_calendar_service():
    """Authenticate with Google Calendar using service account."""
    from google.oauth2.service_account import Credentials
    from googleapiclient.discovery import build

    creds_dict = json.loads(SERVICE_ACCOUNT_JSON)
    creds = Credentials.from_service_account_info(
        creds_dict, scopes=["https://www.googleapis.com/auth/calendar"]
    )
    return build("calendar", "v3", credentials=creds)


from fastapi import FastAPI, Request
from fastapi.responses import JSONResponse

web_app = FastAPI()


@web_app.get("/")
async def health():
    return {"status": "running", "service": "Airbnb Calendar - Book Appointment"}


@web_app.post("/book-appointment")
async def book_appointment(request: Request):
    """Create a booking on the Google Calendar.

    Expects JSON:
    {
        "check_in": "YYYY-MM-DD",
        "check_out": "YYYY-MM-DD",
        "guest_name": "John Smith",
        "guest_email": "john@example.com",
        "guest_phone": "555-1234",
        "group_size": 4
    }
    Returns: {"success": true, "confirmation_number": "...", "message": "..."}
    """
    try:
        body = await request.json()
        check_in = body.get("check_in")
        check_out = body.get("check_out")
        guest_name = body.get("guest_name")
        guest_email = body.get("guest_email", "")
        guest_phone = body.get("guest_phone", "")
        group_size = body.get("group_size", 1)

        # Validate required fields
        if not check_in or not check_out:
            return JSONResponse(
                status_code=400,
                content={"error": "Missing check_in or check_out date. Format: YYYY-MM-DD"}
            )
        if not guest_name:
            return JSONResponse(
                status_code=400,
                content={"error": "Missing guest_name"}
            )

        # Parse and validate dates
        try:
            ci_date = datetime.strptime(check_in, "%Y-%m-%d")
            co_date = datetime.strptime(check_out, "%Y-%m-%d")
        except ValueError:
            return JSONResponse(
                status_code=400,
                content={"error": "Invalid date format. Use YYYY-MM-DD"}
            )

        if co_date <= ci_date:
            return JSONResponse(
                status_code=400,
                content={"error": "check_out must be after check_in"}
            )

        nights = (co_date - ci_date).days
        confirmation = f"MITS-{uuid.uuid4().hex[:8].upper()}"

        service = get_calendar_service()

        # Check for conflicts first
        events_result = service.events().list(
            calendarId=CALENDAR_ID,
            timeMin=f"{check_in}T00:00:00Z",
            timeMax=f"{check_out}T23:59:59Z",
            singleEvents=True,
        ).execute()

        if events_result.get("items"):
            return JSONResponse(
                status_code=409,
                content={
                    "success": False,
                    "error": "Date conflict — those dates are already booked.",
                    "message": "Those dates are not available. There is an existing booking in that range.",
                }
            )

        # Create the calendar event
        event = {
            "summary": f"Mansion In The Sky - {guest_name} ({group_size} guests)",
            "description": (
                f"Confirmation: {confirmation}\n"
                f"Guest: {guest_name}\n"
                f"Email: {guest_email}\n"
                f"Phone: {guest_phone}\n"
                f"Group Size: {group_size}\n"
                f"Nights: {nights}\n"
                f"Booked via: AI Concierge (Alex)"
            ),
            "start": {"date": check_in},
            "end": {"date": check_out},
            "colorId": "9",  # Blueberry
        }

        # Add guest as attendee if email provided
        if guest_email:
            event["attendees"] = [{"email": guest_email}]

        created = service.events().insert(
            calendarId=CALENDAR_ID,
            body=event,
            sendUpdates="all" if guest_email else "none",
        ).execute()

        return {
            "success": True,
            "confirmation_number": confirmation,
            "message": (
                f"Booking confirmed! {guest_name}, party of {group_size}, "
                f"checking in {check_in}, checking out {check_out} ({nights} nights). "
                f"Confirmation number: {confirmation}"
            ),
            "event_id": created.get("id"),
            "check_in": check_in,
            "check_out": check_out,
            "guest_name": guest_name,
            "guest_email": guest_email,
            "guest_phone": guest_phone,
            "group_size": group_size,
            "nights": nights,
        }

    except Exception as e:
        print(f"[BOOK-APPOINTMENT] Error: {e}")
        return JSONResponse(status_code=500, content={"error": str(e)})


@app.function(image=image)
@modal.asgi_app()
def book_appointment_server():
    return web_app
